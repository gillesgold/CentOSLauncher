...
bool DistributionInfo::CreateUser(std::wstring_view userName)
{
	...
	//	usermod : group 'portage' does not exist
	//	usermod : group 'usb' does not exist

	commandLine = L"yum -y update";
	...
	commandLine = L"yum -y install passwd which ncurses git";
	hr = g_wslApi.WslLaunchInteractive(commandLine.c_str(), true, &exitCode);
	if ((FAILED(hr)) || (exitCode != 0)) {
		return false;
	}
	...
	commandLine = L"/usr/sbin/usermod -aG adm,audio,cdrom,wheel ";
	commandLine += userName;
	hr = g_wslApi.WslLaunchInteractive(commandLine.c_str(), true, &exitCode);
	if ((FAILED(hr)) || (exitCode != 0)) {
	...
	// /bin/bash: /usr/bin/passwd: No such file or directory
